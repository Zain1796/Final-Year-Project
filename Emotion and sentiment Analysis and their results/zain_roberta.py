...
for i in zain:
    ...
    tokenizer = tf.AutoTokenizer.from_pretrained('avichr/heBERT_sentiment_analysis')
    model = tf.AutoModelForSequenceClassification.from_pretrained('avichr/heBERT_sentiment_analysis')
    df['sentiment'] = df['tweetContent'].apply(lambda x: analyze_sentiment(x))
    df.to_csv("ResultsRoberta.csv")
